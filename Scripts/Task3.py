...
class TradeBot:
    ...
    def get_stat(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        # Max Drawdown
        # df = weekly_data.min(axis=0)

        # maxdraw = (weekly_data.max(axis=0) - weekly_data.min(axis=0))
        df = []
        cols = weekly_data.columns
        idmax = weekly_data.idxmax()

        for i in cols:
             diff = (weekly_data[i].max() - weekly_data[weekly_data[i].index > idmax[i]][i].min())
             df.append(diff)

        maxdraw = pd.Series(np.array(df), index=cols)

        # for loop
        # weekly_returns[weekly_returns.index > date]

        # Time under water
        df = weekly_data.cummax()
        dfboolean = weekly_data < df
        tuw = dfboolean.sum()

        # VaR and cVaR for Asset Returns
        var = weekly_data.quantile(0.05)
        cvar = weekly_data[weekly_data <= var].mean() # Should it be <= or <

        # Write all results into a data frame
        stat_df = pd.concat([mu_ga, std_dev_a, sharpe, maxdraw, tuw, cvar], axis=1)
        stat_df.columns = [
    ...
    def get_top_performing_assets(
        self, time_periods: List[Tuple[str, str]], top_percent: float = 0.2 #Change
    ) -> List[str]:
        stats_for_periods = {
    ...
    def clustering(
        ...
        n_assets: int,
        plot: bool = False,
    ) -> Tuple[Union[None, px.scatter], list, pd.DataFrame]:
        """
        METHOD TO RUN MST METHOD AND PRINT RESULTS
        """
        fig = None
        ...
        return fig, subset_clustering, clusters
...
if __name__ == "__main__":
    ...
    subset_clustering, clustering_subset_of_assets, clusters = algo.clustering(
        start_date="2013-01-09",
        end_date="2018-01-01",
        n_clusters=6,
        n_assets=8,
    ...
    weekly_returns = weekly_returns[
        (weekly_returns.index >= start_date)
        & (weekly_returns.index <= end_date)
        ]


    # Get the weekly data for the Subset of Isin assets that has been selected from Clustering
    df = weekly_returns[clustering_subset_of_assets]

    covM = np.cov(df, rowvar=False, bias=True)
    print(covM.mean())

    df = algo.get_stat(start_date, end_date)

    #std_dev_a = round(data.std(axis=0) * np.sqrt(52), 2)


    column_names = df.columns

    df = df.loc[clustering_subset_of_assets]
    df = df.loc[df['Average Annual Returns'] > 0]
    clustering_subset_of_assets = list(df.index)
    ...
    covM = np.cov(weekly_returns[clustering_subset_of_assets], rowvar=False, bias=True)
    print(covM.mean())

    # Get the cluster ID data
    clist = []
    for i in clustering_subset_of_assets:
        clist.append(clusters.loc[i]['Cluster'])

    df['Cluster'] = clist

    print(df)
    print(clustering_subset_of_assets)

    # PLOT INTERACTIVE GRAPH
    top_names = [algo.names[algo.tickers.index(isin)] for isin in clustering_subset_of_assets]

    b = algo.plot_dots(
        start_date=algo.min_date, end_date=end_date, fund_set=top_names
    )
    ...
    print("Hello world")
    std_dev_a = np.sqrt(np.diag(covM)) * np.sqrt(52)
    df['Std (CovM)'] = list(std_dev_a)

    # Write to textfile
    with open('selected_portfolio.txt', 'w') as f:
        for line in clustering_subset_of_assets:
            f.write(f"'{line}', ")

    # Write pandas to excel
    df.to_excel("output.xlsx")

    #std_dev_a = round(data.std(axis=0) * np.sqrt(52), 2)

    # Plot the correlation matrix and output to pdf

    # Shouldn't we get sigma^2 in the diagonal? Why is the diagonal also so small???
